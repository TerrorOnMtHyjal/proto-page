...
class OptionsSlider extends Component {
  ...
  generateOptions(items){
    return items.map(item => {
      ...
      if(checkedItem){
        return(
          <OptionButton checked key={`${item}${this.props.type}`} onClick={() => this.props.dispatch(updateActiveFont(item, this.props.type, this.props.loadedMenu))}>
            <p>{formatOption(item)}</p> <i className="fa fa-check fa-lg" aria-hidden="true"></i> 
          </OptionButton>
        );
      } else {
        return (
